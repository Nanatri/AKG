...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'yarışma') {
    msg.reply('Arkadaşlar 2 Tip Çarkımız Var 1. Çark Yarısma Çarkı Kazanılan Ödüller Pazar Günü Saat 19 00 Da Kanalımızda Yapılıcak olan çekilişle 1 kişiye ödülleri dağıtılacak 2. Çark İse Anlıktır Ve Bu Çark Kanala Yayın Anında Her Gelen 4 Abonede Bir Olmaktadır Ve Sadece Sublar Katılmaktadır Burda Çıkan Ödüller İse her ayın 1inde Teslim Edilicektir Tesekkürler.');
  }
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'iyi geceler') {
    msg.reply('iyi geceler tabi böyle bir şey mümkünse :( ');
  }
});
 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak! **AKG Bot'un kıymetini bil :D**")
...
client.on("channelCreate", async (channel, member, guild) => {
  ...
  if (kanal == "acik") {
    ...
    const embed = new Discord.RichEmbed()
      .setDescription(
        "Sunucunuzda yeni bir kanal oluşturuldu! fakat geri silindi! ( Kanal Koruma Sistemi) "
      )
      .setColor("BLACK");
